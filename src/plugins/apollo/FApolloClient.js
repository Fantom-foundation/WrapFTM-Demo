...
export class FApolloClient {
    /**
     * @param apolloProviders
     * @param {string} [defaultProviderIndex]
     * @param {number} [maxRetryLinkAttempts]
     * @param {Auth} [auth]
     */
    constructor({
        apolloProviders = [],
        defaultProviderIndex = 'random',
        maxRetryLinkAttempts = Infinity,
        auth = null,
    }) {
        ...
        if (defaultProviderIndex === 'random') {
            this.defaultProviderIndex = Math.floor(
                Math.random() * this.apolloProviders.length
            );
        }
        ...
        this.httpApolloProviders = this.setHttpApolloProviders(
            this.apolloProviders,
            this.defaultHttpProvider
        );
    ...
    getHttpAuthLink() {
        return setContext((operation, { headers }) => {
            const token = this.auth ? this.auth.getBearerToken() : null;
            ...
            attempts: {
                ...
                retryIf: (_error, _operation) => {
                    ...
                    if (
                        ...
                        (!this.lastOperationName ||
                            _operation.operationName === this.lastOperationName)
                    ) {
